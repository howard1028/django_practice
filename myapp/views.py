...
def index(request):
    ...
    features = Feature.objects.all() #a list
    return render(request , 'index.html' , {'features':features}) #render:get from
...
def register(request):
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists(): #database User
